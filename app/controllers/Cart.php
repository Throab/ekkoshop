...
class Cart extends Controller
{
    ...
    function addCartApi()
    {
        ...
        if (!empty($dataCartItem)) {
            ...
            $updateCart = $this->cartModel->updateCart($dataCart['id'], [
                ...
                'total_weight' => $dataCart['total_weight'] + ($dataProductVariant['weight'] * $dataPost['quantity']),
    ...
    function updateQuantityApi($id, $action)
    {
        ...
        if ($action == 'plus') {
            ...
            $newTotalWeight = $dataCartItem['total_weight'] + $dataCartItem['weight'];
    ...
    function deleteCartApi($id)
    {
        ...
        $newTotalWeight = $dataCartItem['total_weight'] - ($dataCartItem['weight'] * $dataCartItem['quantity']);
        $updateCart = $this->cartModel->updateCart($dataCartItem['cart_id'], [
            'totalPrice' => $newTotalPrice,
            'total_weight' => $newTotalWeight,
            'update_at' => date('Y-m-d H:i:s'),
