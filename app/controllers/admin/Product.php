...
class Product extends Controller
{
    ...
    function addProduct()
    {
        ...
        if (isset($dataPost['attribute']) && !empty($dataPost['attribute'][0])) {
            ...
            foreach ($dataPost['price_variant'] as $keyVariant => $price_variant) {
                ...
                if(!empty($dataPost['weight_variant'][$keyVariant])){
                    $dataInsertVariant[$keyVariant]['weight'] = $dataPost['weight_variant'][$keyVariant];
                }else{
                    $this->Toast('error','Vui lòng không để trống trọng lượng');
                    return $this->renderAddPage($cateData, $brandData, $attributeData, $attributeValueData, $dataValueOld);
                }
            }
        ...
        if (isset($dataPost['product_variants_id']) && !empty($dataPost['product_variants_id'][0])) {
            ...
            foreach ($dataPost['product_variants_id'] as $keyVariantId => $product_variants_id) {
                $dataUpdateVariants[] = [
                    ...
                    'weight' => $dataPost['weight'][$keyVariantId]
                ];
                ...
                if(empty($dataPost['discount'][$keyVariantId])){
                    return $this->res->setToastSession('error', 'Vui lòng nhập trọng lượng', 'admin/product-variants/' . $id);
                }else{
                    $dataUpdateVariants[$keyVariantId]['weight'] = $dataPost['weight_variant'][$keyVariantId];
                }
            }
        }
    ...
    function addProductVariants($id)
    {
        ...
        foreach ($dataPost['price_variant'] as $keyVariant => $price_variant) {
            ...
            if(empty($dataPost['weight_variant'][$keyVariant])){
                return $this->res->setToastSession('error', 'Vui lòng nhập trọng lượng', 'admin/add-product-variants/' . $id);
            }else{
                $dataInsertVariants[$keyVariant]['weight'] = $dataPost['weight_variant'][$keyVariant];
            }
        }
